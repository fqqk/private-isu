...
module Isuconp
  class App < Sinatra::Base
    ...
    get '/' do
      ...
      # 全件取得してしまっている
      old_query = 'SELECT `id`, `user_id`, `body`, `created_at`, `mime` FROM `posts` ORDER BY `created_at` DESC'\
    ...
    post '/' do
      ...
      if params['file']
        ...
        if params["file"][:type].include? "jpeg"
          # mime = "image/jpeg"
          mine, ext = "image/jpeg", "jpg"
        elsif params["file"][:type].include? "png"
          # mime = "image/png"
          mine, ext = "image/png", "png"
        elsif params["file"][:type].include? "gif"
          # mime = "image/gif"
          mine, ext = "image/gif", "gif"
        else
        ...
        # アップロードされたテンポラリファイルをmvして配信ディレクトリに移動する
